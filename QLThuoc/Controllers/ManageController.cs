...
#region Helpers
        ...
        [Authorize(Roles = "Trạm trưởng")]
        public ActionResult Accounts(string search)
        {
            if (search != null)
            {
                ViewBag.Search = search;
                return View(db.AspNetUsers.Where(x => ((x.Email.Contains(search) || x.UserName.Contains(search) || x.PhoneNumber.Contains(search)) && x.Disable == false)).ToList());
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,Email,EmailConfirmed,PasswordHash,SecurityStamp,PhoneNumber,PhoneNumberConfirmed,TwoFactorEnabled,LockoutEndDateUtc,LockoutEnabled,AccessFailedCount,UserName,Disable")] AspNetUser aspNetUser)
        {
            if (ModelState.IsValid)
            {
